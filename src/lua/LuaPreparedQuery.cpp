...
MYSQLOO_LUA_FUNCTION(setNumber) {
    ...
    // Validate index is positive and within reasonable range
    if (index < 1.0 || index > UINT_MAX) {
        LUA->ThrowError("Index must be between 1 and UINT_MAX");
        return 0;
    }
...
MYSQLOO_LUA_FUNCTION(setString) {
    ...
    // Validate string length is reasonable (prevent extremely large strings)
    const size_t MAX_STRING_LENGTH = 16777215; // MEDIUMTEXT max length
    if (length > MAX_STRING_LENGTH) {
        LUA->ThrowError("String length exceeds maximum allowed length");
        return 0;
    }
    auto uIndex = (unsigned int) index;
