 ...
 */
size_t Database::queueSize() const {
...
std::string Database::escape(const std::string &str) {
    ...
    //Check for potential integer overflow before buffer allocation
    if (str.size() > SIZE_MAX / 2) {
        throw MySQLOOException("String too large to escape");
    }
    std::vector<char> escapedQuery(str.size() * 2 + 1);
    unsigned long nEscapedQueryLength = mysql_real_escape_string(m_sql, escapedQuery.data(), str.c_str(),
 ...
 */
unsigned int Database::serverVersion() const {
    if (!m_connectionDone) {
 ...
 * Only works as soon as the connection has been established
 */
std::string Database::serverInfo() const {
    if (!m_connectionDone) {
 ...
 * Only works as soon as the connection has been established
 */
std::string Database::hostInfo() const {
    if (!m_connectionDone) {
        throw MySQLOOException("Tried to get server info when client is not connected to server yet!");
    }
    return m_hostInfo;
...
void Database::runQuery(const std::shared_ptr<IQuery>& query, const std::shared_ptr<IQueryData>& data, bool retry) {
    const unsigned int MAX_RETRIES = 1; // Only retry once to prevent infinite loops
    unsigned int retryCount = 0;
    bool shouldRetry = retry;
    ...
    while (true) {
        try {
            ...
            if (shouldRetry && retryableError && retryCount < MAX_RETRIES && attemptReconnect()) {
                ...
                shouldRetry = false; // Only allow one retry
                retryCount++;
...
bool Database::attemptReconnect() {
    my_bool reconnect = '1';
    bool success = mariadb_reconnect(this->m_sql) == 0;
    reconnect = '0';
    // Disable automatic reconnection to prevent infinite reconnection loops
    if (mysql_optionsv(this->m_sql, MYSQL_OPT_RECONNECT, &reconnect) != 0) {
        // If setting the option fails, connection is likely invalid anyway
        return false;
    }
    return success;
}
