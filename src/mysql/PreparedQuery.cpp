...
void PreparedQuery::setNumber(unsigned int index, double value) {
    ...
    m_parameters.back()[index] = std::make_shared<TypedQueryField<double>>(index, MYSQL_TYPE_DOUBLE, value);
}
...
void PreparedQuery::setString(unsigned int index, const std::string &value) {
    ...
    m_parameters.back()[index] = std::make_shared<TypedQueryField<std::string>>(index, MYSQL_TYPE_STRING, value);
}
...
void PreparedQuery::setBoolean(unsigned int index, bool value) {
    ...
    m_parameters.back()[index] = std::make_shared<TypedQueryField<bool>>(index, MYSQL_TYPE_BIT, value);
}
    ...
    try {
        ...
        for (auto &currentMap: data->m_parameters) {
            ...
            do {
                ...
                //Handle potential failure when storing result after successful execution
                //If mysqlStmtStoreResult fails, the statement executed but we can't store results
                //The finally handler will clean up metaData, and the exception will be caught by outer handler
                mysqlStmtStoreResult(stmt);
                auto f2 = finally([&] { mysql_stmt_free_result(stmt); });
                //If emplace_back throws, the finally handlers will clean up metaData and stmt properly
