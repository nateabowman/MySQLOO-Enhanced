...
ResultData::ResultData(MYSQL_RES* result) : ResultData((unsigned int)mysql_num_fields(result), (unsigned int)mysql_num_rows(result)) {
	...
	while ((currentRow = mysql_fetch_row(result)) != nullptr) {
		...
		if (lengths == nullptr) {
			throw MySQLException(0, "Failed to fetch row lengths");
		}
...
ResultData::ResultData(MYSQL_STMT* result, MYSQL_RES* metaData) : ResultData((unsigned int)mysql_stmt_field_count(result), (unsigned int)mysql_stmt_num_rows(result)) {
	...
	for (unsigned int i = 0; i < columnCount; i++) {
		...
		unsigned long maxLength = fields[i].max_length;
		if (maxLength > SIZE_MAX - 2) {
			throw MySQLException(0, "Field max_length too large for buffer allocation");
		}
		buffers.emplace_back(maxLength + 2);
		bind.buffer = buffers.back().data();
		bind.buffer_length = maxLength + 1;
