...
MYSQLOO_LUA_FUNCTION(escape) {
    ...
    const size_t MAX_STRING_LENGTH = SIZE_MAX / 2; // Match the limit in Database::escape
    if (nQueryLength > MAX_STRING_LENGTH) {
        LUA->ThrowError("String length exceeds maximum allowed length for escaping");
        return 0;
    }
